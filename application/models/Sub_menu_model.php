...
class Sub_menu_model extends CI_Model {
    ...
    function reload_order_by_pos($menuid) {
        $this->db->where('menuid', $menuid);
		$this->db->from($this->table);
        $data = $this->db->get();
        return $data->result();
    ...
    function save($menuid, $active, $pos, $name) {
        $data = array(
            'menuid' => $menuid,
            'active' => $active,
            'position' => $pos,
    ...
    public function count_filtered($menuid) { 
        $this->db->where('menuid', $menuid);    
	...
	public function get_by_id($id) {    
        $query = $this->db->query("select e.*, u1.name as firstname, u2.name as lastname FROM sub_menu e LEFT JOIN users u1 ON e.userfirst = u1.id LEFT JOIN users u2 ON e.userlast = u2.id WHERE e.id=".$id); 		
        return $query->row();
    }	
    
    public function view_by_menuid($menuid) {
        $this->db->where('menuid', $menuid);
