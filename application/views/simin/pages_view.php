		...
		<script type="text/javascript">
      		...
      		$(document).ready(function() {
				...
				table = $('#table').DataTable({ 
					...
					"ajax": { "url": "<?php echo site_url('simin/pages/ajax_list')?>", "type": "POST" }, // Load data for the table's content from an Ajax source
					"columnDefs": [			
				...
				bootbox.confirm({
					...
					callback: function (result) {
						if(result) {
							...
							$.ajax({
								url : "<?php echo site_url('simin/pages/delete')?>/"+id,
								type: "POST",
								dataType: "JSON",
			...
			function edit_record(id) {
				window.location.replace("/simin/pages/rec_edit/"+id);
			} 

			function dup_record(id) {
				$.ajax({
					url : "<?php echo site_url('simin/pages/rec_dup')?>/"+id,
						type: "POST",
						dataType: "JSON",
						success: function(data) {	
							reload_table();																		
					...
					callback: function (result) {
						if (result) {
							...
							if(id.length === 0) {
								...
								// ajax delete data to database
								$.ajax({
									url : "<?php echo site_url('simin/pages/del_selected')?>",
									type: "POST",
									dataType: "JSON",
									data:{id:id},
									success: function(data) {	
