	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$('#select_property').change(function() {
				var id = $(this).val();
				$.ajax({
					url: "<?php echo base_url(); ?>simin/membership/change_property/",
					method: "POST",
					data: {
						id: id
					},
			...
			table = $('#table').DataTable({
				...
				"ajax": {
					"url": "<?php echo site_url('simin/membership/ajax_list') ?>",
					"type": "POST"
		...
		function add_record() {
			...
			$('.modal-title').text('Add Membership'); // Set Title to Bootstrap modal title
            $('#btncard').hide();
		}
		...
		function save() {
			...
			if (save_method == 'add') {
				url = "<?php echo site_url('simin/membership/add_save') ?>";
			} else {
				url = "<?php echo site_url('simin/membership/ajax_update') ?>";
			}
			...
			bootbox.confirm({
				...
				callback: function(result) {
					if (result) {
						...
						$.ajax({
							url: "<?php echo site_url('simin/membership/delete') ?>/" + id,
							type: "POST",
							dataType: "JSON",
							success: function(data) {
								reload_table();
		...
		function edit_record(id) {
			...
			$.ajax({
				url: "<?php echo site_url('simin/membership/ajax_edit/') ?>/" + id,
				type: "GET",
				dataType: "JSON",
				success: function(data) {
                    ...
                    $('[name="memberid"]').val(data.member_id);
				...
				callback: function(result) {
					if (result) {
						...
						if (id.length === 0) {
							...
							// ajax delete data to database
							$.ajax({
								url: "<?php echo site_url('simin/membership/del_selected') ?>",
								type: "POST",
								dataType: "JSON",
								data: {
									id: id
		...
		function view_order(id) {
			window.location.replace("/simin/membership/order/" + id);
		}

        function view_member_card(id) {
            window.location.replace("/simin/membership/card/" + id);
        }
	</script>
		...
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
					...
					<form action="#" id="form" class="form-horizontal">
						<div class="kt-section__body">
                            ...
                            <div class="form-group row">
								...
								<div class="col-sm-8 input-group date">
									<input type="text" class="form-control" name="memberid" required="true" id="memberid" value = "AUTO" readonly placeholder="Phone / mobile no">
									<span class="error invalid-feedback"></span>
								</div>
							</div>
							<div class="form-group row">
				...
				<div class="modal-footer">
                    ...
                    <button type="button"id="btncard" onclick="view_member_card(member_id)" class="btn btn-warning btn-sm">View Card</button>
				</div>
			</div><!-- /.modal-content -->
		</div><!-- /.modal-dialog -->
	</div><!-- /.modal -->
