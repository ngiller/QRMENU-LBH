...
class Print_order extends CI_Controller
{
    ...
    function print_id($id)
    {
        ...
        $text_print = "";
        $text_print .= $this->str_center($this->session->outletname, $text_length) . "\n";
        $text_print .= "--------------------------------\n";
        ...
        foreach ($detail as $item) {
            ...
            if ($this->order_detail_item_model->has_item($order->id, $item->menuid) > 0) {
                $submenu = $this->order_detail_item_model->get_by_order($order->id, $item->menuid);
    ...
    /*public function set_already_print($id)
    {
        $id = xssclean($id);
        $data = array(
            'print' => 0
    ...
    {
        $order_id = array();
        $print = $this->order_master_model->get_print();
        $text_print = "";
        foreach ($print as $item) {
            $text_print .= $this->print_id($item->id) . "\n\n\n\n";
            $order_id[] = $item->id;
            //$this->set_already_print($item->id);
        }
        $data = array(
            "status" => TRUE,
            "order_id" => $order_id,
    ...
    {
        $order_id = array();
        $print = $this->order_print_model->get_print();
        $text_print = "";
        foreach ($print as $item) {
